...
type ScanSource = { tool: string; path: string; skillFile?: string; ownership: string; contentHash?: string }
type ScanGroup = { id: string; status: 'discovered' | 'duplicate' | 'conflict'; sourceCount: number; duplicateCount: number; contentHashes: string[]; sources: ScanSource[]; packageId?: string }
  ...
  return (
    <div className="skills-page">
      ...
      <Card>
        ...
        <CardContent><div className="skills-list">{scan.filter(item => !importedIds.has(item.id)).slice(0, 150).map(item => <div className="skill-row" key={item.id}><div><strong>{item.id}</strong><small>{item.sourceCount} {zh ? '个来源' : 'sources'} · {item.status === 'conflict' ? (zh ? '内容冲突，请选择版本' : 'content conflict; choose a version') : item.status === 'duplicate' ? (zh ? '重复来源已合并' : 'identical sources merged') : (zh ? '待导入' : 'ready to import')}</small>{item.sources.length > 1 && <select value={sourceChoice[item.id] || item.sources[0].path} onChange={event => setSourceChoice(previous => ({ ...previous, [item.id]: event.target.value }))}>{item.sources.map(source => <option key={`${source.tool}:${source.path}`} value={source.path}>{source.tool} · {source.contentHash?.slice(-12) || source.path}</option>)}</select>}</div><div className="skill-row-state">{item.status === 'conflict' ? <span className="skill-warning"><ShieldAlert size={15} />{zh ? '需选择' : 'Choose version'}</span> : item.status === 'duplicate' ? <span className="skill-good"><CheckCircle2 size={15} />{zh ? '已去重' : 'Deduplicated'}</span> : null}<Button size="sm" onClick={() => void importSkill(item)} disabled={busy}><Download size={14} />{item.status === 'conflict' ? (zh ? '导入所选版本' : 'Import selected') : (zh ? '导入到 AMH' : 'Import to AMH')}</Button></div></div>)}{!scan.length && <div className="skills-empty">{zh ? '未发现本机 Skill。' : 'No local Skills discovered.'}</div>}</div></CardContent>
      </Card>
      <Card>
