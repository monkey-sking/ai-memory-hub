...
这是比“规则还是 Skill”更优先的一层判断：

| 范畴 | 可以进入公开代码仓库 | 不应进入公开代码仓库 |
| --- | --- | --- |
| 项目本身 | 源代码、测试、接口定义、脱敏示例、规范、模板、迁移脚本 | 用户身份、机器路径、账号标识、真实项目数据 |
| 项目知识 | 产品规则、UX Policy、技术决策、质量门禁、可复用 Skill/Playbook | 某次任务的原始上下文、未脱敏聊天和内部审查记录 |
| 项目产生的数据 | 仅可提交经过明确脱敏、可复现且确实属于测试夹具的数据 | MEMORY、任务、Radio、Workflow、日志、运行快照、凭据、真实飞书文档内容和业务报表 |
| 项目配置 | `config.example.json`、公开默认值、版本锁定的 Skill 清单 | `config.json`、Token、Cookie、密钥、个人 Agent 配置 |

公开仓库 `monkey-sking/ai-memory-hub` 只承载项目本身和可公开知识，不承载该项目运行后产生的个人/业务数据。运行数据应写入本机共享目录、项目本地被忽略目录或用户明确指定的受控存储；上传前必须经过脱敏和人工确认。

因此，实际飞书文档内容、文档链接、项目参数和用户偏好不是同一类东西：文档编辑方法可以成为 Skill，文档格式偏好可以成为 Preference/Policy，而某个文档的内容和链接属于项目数据或外部业务数据。
