...
| Collaboration state | Shared handoffs, tasks, workflow plans, reviews, approvals, and radio messages that coordinate tools. | `radio/messages.jsonl`, `tasks/events.jsonl`, `tasks/tasks.jsonl`, `workflows/events.jsonl`, `workflows/workflows.jsonl`, `projects/events.jsonl`, `projects/projects.jsonl`, sessions, notifications, context packs | May be updated through task/workflow/radio/project APIs. Event files are the write source for tasks, workflows, and projects; projection files are rebuilt compatibility views. This is auditable working state, not startup memory by default. |
