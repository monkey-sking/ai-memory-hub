...
export function backupCommand(argv, deps) {
  ...
  if (action === "prune") {
    ...
    const prePull = getOption(argv, "--pre-pull")
      ...
      : retention.prePull;
    const adHoc = getOption(argv, "--ad-hoc")
      ? parsePositiveIntegerOption(getOption(argv, "--ad-hoc"), "--ad-hoc")
      : retention.adHoc;
    const result = apply
      ? deps.withHubLock(config.memoryDir, "backup-prune", () => deps.pruneBackups(config.memoryDir, { apply, daily, weekly, preSync, prePull, adHoc }), config.sync.lockStaleMs)
      : deps.pruneBackups(config.memoryDir, { apply, daily, weekly, preSync, prePull, adHoc });
    console.log(JSON.stringify(result, null, 2));
    return;
  }
  if (action !== "create") {
    throw new Error("Usage: ai-memory-hub backup [--reason manual] | ai-memory-hub backup status | ai-memory-hub backup run [--no-push] | ai-memory-hub backup configure [--enabled] [--remote-url <url>] [--repo-dir <dir>] [--allow-plaintext-sensitive] | ai-memory-hub backup schedule <status|install|uninstall> | ai-memory-hub backup list [--limit N] | ai-memory-hub backup prune [--daily 7] [--weekly 4] [--pre-sync 5] [--pre-pull 5] [--ad-hoc 10] [--apply]");
  }
