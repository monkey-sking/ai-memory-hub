...
export async function scanSkillRoots(roots = []) {
  ...
  for (const entry of roots) {
    ...
    await walk(root, async (directory) => {
      ...
      results.push({ id, path: directory, skillFile, tool, contentHash: `sha256:${(await import("node:crypto")).createHash("sha256").update(content, "utf8").digest("hex")}`, ownership: marker ? "amh-managed" : "user", packageId: pack?.id || "", packageVersion: pack?.version || "", packagePath: pack?.path || "" });
    });
  }
...
export function aggregateSkillSources(results = []) {
  ...
  for (const source of results) {
    ...
    group.status = group.conflict ? "conflict" : group.duplicateCount > 0 ? "duplicate" : "discovered";
    groups.set(id, group);
  }
