---
name: ai-memory-hub
description: Local shared AI memory hub for Coze (扣子). Use when the user mentions AI Memory Hub, shared memory, cross-agent memory, Agent Radio, 本地记忆, 共享记忆, 多 AI 同步, or asks Coze to remember durable preferences/project facts across AI tools.
version: 0.1.0
metadata: {"category":"memory","keywords":["AI Memory Hub","shared memory","Agent Radio","local ledger","本地记忆","共享记忆","多AI同步","扣子","Coze"]}
---
...
```json
{"source":"coze","text":"short durable memory","metadata":{"kind":"preference|project|workflow|correction"}}
```
...
ai-memory-hub task add "short task title" --from coze --project <project> --priority normal
ai-memory-hub task claim --id <task-id> --by coze
ai-memory-hub task note --id <task-id> "handoff note or progress update" --by coze
ai-memory-hub task done --id <task-id> --by coze
```
...
ai-memory-hub workflow create "short workflow title" --from coze --project <project> --planner coze --executor <tool> --reviewer <tool> --spawn-tasks
ai-memory-hub workflow start --id <workflow-id> --by coze
ai-memory-hub workflow result --id <workflow-id> --role executor "execution result" --by coze
ai-memory-hub workflow review --id <workflow-id> --role reviewer "review result" --by coze
ai-memory-hub workflow signal --id <workflow-id> --to <role-or-tool> "handoff note" --by coze
```
...
```json
{"source":"coze","from":"coze","to":"all","type":"note","text":"short cross-agent message"}
```
...
ai-memory-hub connect request --from coze --to codex --project <project> --text "please inspect this task" --task
ai-memory-hub connect review --from coze --to claude --project <project> --text "please review this change" --task
ai-memory-hub connect handoff --from coze --to qclaw --project <project> --text "handoff context and next step" --task
...
ai-memory-hub sync
ai-memory-hub backup --reason coze
ai-memory-hub task list --status active
ai-memory-hub workflow list --status active
...
Coze (扣子) is a cloud-based AI agent platform. When running on a user's local machine via desktop integration:
- Use the local `ai-memory-hub` CLI for all memory operations
- The Coze agent can act as a planner, reviewer, or executor in workflows
- Coze excels at web research, document writing, data analysis, and multi-step planning
- For code-heavy tasks, consider handing off to codex or claude via shared tasks
...
curl -s -X POST http://127.0.0.1:38787/api/dispatch/marvis ^
  ...
  -d "{\"text\":\"Find all PDF invoices in D:\\Documents and convert them to a single Excel file at D:\\Documents\\invoices.xlsx\",\"from\":\"coze\",\"project\":\"<project-name>\"}"
```
