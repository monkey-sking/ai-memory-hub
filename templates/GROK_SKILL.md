---
name: ai-memory-hub
description: Local shared AI memory hub for Grok (xAI). Use when the user mentions shared memory, cross-agent memory, Agent Radio, local memory, shared memory across AI tools, or asks Grok to remember durable preferences/project facts.
version: 0.1.0
metadata: {"category":"memory","keywords":["shared memory","Agent Radio","local ledger","local memory","cross-agent memory","grok","xai"]}
---
...
```json
{"source":"grok","text":"short durable memory","metadata":{"kind":"preference|project|workflow|correction"}}
```
...
ai-memory-hub task add "short task title" --from grok --project <project> --priority normal
ai-memory-hub task claim --id <task-id> --by grok
ai-memory-hub task note --id <task-id> "handoff note or progress update" --by grok
ai-memory-hub task done --id <task-id> --by grok
```
...
ai-memory-hub workflow create "short workflow title" --from grok --project <project> --planner grok --executor <tool> --reviewer <tool> --spawn-tasks
ai-memory-hub workflow start --id <workflow-id> --by grok
ai-memory-hub workflow result --id <workflow-id> --role executor "execution result" --by grok
ai-memory-hub workflow review --id <workflow-id> --role reviewer "review result" --by grok
ai-memory-hub workflow signal --id <workflow-id> --to <role-or-tool> "handoff note" --by grok
```
...
```json
{"source":"grok","from":"grok","to":"all","type":"note","text":"short cross-agent message"}
```
...
ai-memory-hub connect request --from grok --to codex --project <project> --text "please inspect this task" --task
ai-memory-hub connect review --from grok --to codex --project <project> --text "please review this change" --task
ai-memory-hub connect handoff --from grok --to codex --project <project> --text "handoff context and next step" --task
ai-memory-hub dispatch --to codex --project <project>
ai-memory-hub dispatch --to codex --project <project> --run
ai-memory-hub dispatch --to grok --project <project> --run
```

Use `--run` only for targets with verified CLI runners (including `grok` when the binary is on PATH or at `~/.grok/bin/grok`). Without `--run`, the request remains shared local state until the target tool reads it.

## Grok headless dispatch

AMH dispatches to Grok with:

```bash
grok --always-approve -p "<compact prompt>"
```

Identity in shared state: always use tool name `grok` for `--source` / `--from` / `--by`.
...
ai-memory-hub sync
ai-memory-hub backup --reason grok
ai-memory-hub task list --status active
ai-memory-hub workflow list --status active
ai-memory-hub radio list --limit 10
ai-memory-hub doctor --tool grok
```
...
curl -s -X POST http://127.0.0.1:38787/api/dispatch/marvis ^
  ...
  -d "{\"text\":\"Find all PDF invoices in D:\\Documents and convert them to a single Excel file at D:\\Documents\\invoices.xlsx\",\"from\":\"grok\",\"project\":\"<project-name>\"}"
```
